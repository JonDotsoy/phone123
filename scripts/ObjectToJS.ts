...
class ObjectToJS {
    ...
    println = (body: string | Buffer) => Buffer.from(`${this.spacing.toString()}${body}\n`);
    print = (body: string) => Buffer.from(`${this.spacing.toString()}${body}`);
    printStart = (body: string) => Buffer.from(`${body}\n`);

    valueToBuffer = (val: any) => isObject(val) ? this.objectToBuffer(val) : Buffer.from(JSON.stringify(val));

    propValueToBuffer = (prop: string, val: any) => {
        if (isArray(val)) {
            if (val.every(e => !isArray(e) && !isObject(e))) {
                return Buffer.concat([
                    this.println(`/** @type {[${val.map(val => JSON.stringify(val)).join(' | ')}]} */`),
                    this.println(`${JSON.stringify(prop)}: ${this.valueToBuffer(val)},`),
                ]);
            }
        }
        if (isObject(val)) {
            return this.println(`${JSON.stringify(prop)}: ${this.valueToBuffer(val)},`);
        }
